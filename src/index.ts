  ...
  execute: async ({
    ...
    try {
      ...
      return {
        ...
        attribution: "Generated by JobRelay — Enso Labs (ensolabs.ai)",
      };
    } catch (err: any) {
      return {
        success: "false",
  ...
  execute: async ({
    ...
    try {
      ...
      return {
        ...
        attribution: "Powered by JobRelay — Enso Labs (ensolabs.ai)",
      };
    } catch (err: any) {
      return {
        success: "false",
...
worker.tool("configureAgent", {
  ...
  description:
    "Update the user's JobRelay preferences: automation mode, scan cadence, score threshold, active job boards, and notification channels. Returns the confirmed configuration.",
  schema: j.object({
  ...
  execute: async ({
    ...
    return {
      ...
      summary: `JobRelay configured: ${mode} mode, scanning ${boards.join(" + ")} ${freq === "on-demand" ? "on demand" : cronMap[freq]}. Threshold: ${threshold}+. Notifications via ${channels.join(" + ")}.`,
      nextStep:
        freq === "on-demand"
...
worker.tool("getAgentStatus", {
  ...
  description:
    "Get a summary of the JobRelay's current state: configuration, recent matches, pipeline counts, and next scheduled scan. Use when the user asks 'what's new', 'status', 'how's my search going', or 'dashboard'.",
  schema: j.object({
